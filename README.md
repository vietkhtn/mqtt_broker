...
1. Prepare TLS material for your domain (for example `test.de`). Place the PEM bundle inside `certs/<domain>.pem`; the file is mounted into the HAProxy container at `/etc/haproxy/certs`.
2. Render the HAProxy configuration for your domain:
   ...
   The script simply generates `haproxy.cfg` from `haproxy.cfg.template`, injecting the domain so every backend points at `*.test.de`. You can also call it as `HOST_DOMAIN=test.de ./start.sh`.
